...
cardsList.forEach(function(current) {
    ...
    current.addEventListener('click', function() {
        ...
        if (playable && current.classList.value !== 'board_item checked' && current.firstChild.classList.value !== 'board_img uncovered') { // checks if the game staturs is playable and if the card you clicked on isnt already guessed
            ...
            if (flippedCards.length === 2) { // checks if there are two flipped cards at the moment
                ...
                if (flippedCards[0].firstChild.src === flippedCards[1].firstChild.src) { // compares the src of the two flipped cards to see if they match
                    ...
                    setTimeout(function() {
                        ...
                        flippedCards = [];
                        playable = true;
                        document.querySelector('.header_p' + currentPlayer + 'label').classList.toggle('active');
...
function randomizer(arr) {
    ...
    for (var i = 0; i < arr.length; i++) { // and then removes it
        if (arr[i] === card) {
            arr.splice(i, 1);
        }
    }
